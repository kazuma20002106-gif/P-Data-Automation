...
def push_to_github():
    ...
    try:
        ...
        print(f"❌ 同期エラー: {e}")
        print("ヒント: Gitがインストールされ、認証（ログイン）が完了しているか確認してください。")
...
def patrol_dai_list(page, dais):
    """全台を執念深く巡回してデータを強奪"""
    first_dai = dais[0]
    print(f"台番号を検索・クリック中: {first_dai}")
    ...
    for i, dai in enumerate(dais):
        ...
        try:
            # "BONUS"というテキストが画面に出現するまで待機
            page.wait_for_selector("text=BONUS", timeout=15000)
        except:
            print("データ描画の待機がタイムアウトしました。既に描画済みの可能性を考慮し抽出を続行します。")
...
def main():
    with sync_playwright() as p:
        # ローカル実行のためブラウザを表示（動作を確認できるようにするため）
